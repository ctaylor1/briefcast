...
```bash
docker pull ghcr.io/ctaylor1/briefcast:1.2.2
...
docker run -d \
  ...
  ghcr.io/ctaylor1/briefcast:1.2.2
```

`latest` should point to the current release tag (`1.2.2`).
...
docker buildx build --platform linux/amd64 --build-arg INSTALL_WHISPERX=true \
  -t ghcr.io/ctaylor1/briefcast:1.2.2 \
...
```bash
git tag -a v1.2.2 -m "Briefcast v1.2.2"
```
