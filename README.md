...
```bash
docker pull ghcr.io/ctaylor1/briefcast:1.1.0
...
docker run -d \
  ...
  ghcr.io/ctaylor1/briefcast:1.1.0
```

`latest` should point to the current release tag (`1.1.0`).
...
- For `v1.1.0`, publish container tags `ghcr.io/ctaylor1/briefcast:1.1.0` and `ghcr.io/ctaylor1/briefcast:latest` from the same image digest.
...
docker buildx build --platform linux/amd64 --build-arg INSTALL_WHISPERX=true \
  -t ghcr.io/ctaylor1/briefcast:1.1.0 \
...
```bash
git tag -a v1.1.0 -m "Briefcast v1.1.0"
```
