...
```bash
docker pull ghcr.io/ctaylor1/briefcast:1.9.2
...
docker run -d \
  ...
  ghcr.io/ctaylor1/briefcast:1.9.2
```
...
The package version is defined in `pyproject.toml`. Current version: `1.9.2`.
...
gh workflow run release.yml --ref <default-branch> -f version=1.9.2
```
...
docker buildx build --platform linux/amd64 --build-arg INSTALL_WHISPERX=true \
  -t ghcr.io/ctaylor1/briefcast:1.9.2 \
...
git tag -a v1.9.2 -m "Briefcast v1.9.2"
